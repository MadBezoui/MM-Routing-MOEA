...
def run_experiments():
    ...
    plans = build_smart_plans(profiles_df, sms_seeds=5) # Reduced seeds for computation speed
    
    logger.info(f"Loading multimodal graph from {args.graph_path}...")
    G = nx.read_graphml(args.graph_path)
    
    # Get all valid osm nodes (we route between walk nodes, not transit stops directly)
...
if __name__ == "__main__":
    ...
    sys.setrecursionlimit(5000) # Prevents recursion depth issues with large graphs
    run_experiments()
